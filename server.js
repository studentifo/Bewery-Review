...
app.post("/login", urlencodedParser, (req, res) => {
  con.query(`SELECT username, password FROM users WHERE username = '${req.body.username}';`, (err, result) => {
    if (err) {
      res.render("index", { errStr: "Invalid Username or password... Try again." });
    } else {
      if (result.length > 0) {
        bcrypt.compare(req.body.password, result[0]["password"], (err, cryptRes) => {
          if (err) throw err;
          if (cryptRes) {
...
app.post("/signup", urlencodedParser, (req, res) => {
  bcrypt.hash(req.body.password, saltRounds).then((hash) => {
    con.query(
      `INSERT INTO users (username, password, fullname, email) VALUES ('${req.body.username}', '${hash}', '${req.body.fullname}', '${req.body.email}');`,
      (err) => {
        if (err) {
